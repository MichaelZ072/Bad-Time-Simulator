...
class Sans 
{
    ...
    public:
        ...
        // displays sans on render window screen
        void draw(RenderWindow* win) {
            win->draw(leg);
            win->draw(body);
            win->draw(head);

            // displays speech when activated
            if (speechActive) {
        ...
        // activate speech and display custom text
        void talk(string dialogueString) {
            this->dialogueString = dialogueString;
            text.setString(dialogueString);
            speechActive = true;
        }
        ...
        // customise his appearance using these functions
        ...
        // moves his whole body in one function
        void moveFull(double offsetX, double offsetY) {
            head.move(offsetX, offsetY);
            body.move(offsetX, offsetY);
            leg.move(offsetX, offsetY); 
        }

        // sans will only be called to dodge to the left
        void dodge(double distance, double endPosition) {
